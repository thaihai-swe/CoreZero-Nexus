...
3. Fill in the adopter-owned project docs under `core-zero/project/` with known facts. Use `[UNKNOWN]` where the answer is not known yet.
...
- Coding standards in `core-zero/rules/` (syntax, language-specific lints, and security standards) must be strictly adhered to.
