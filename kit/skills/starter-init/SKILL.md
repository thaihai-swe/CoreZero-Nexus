...
3. **Step 3: Memory Customization (Phase B)**: This is mandatory. The installer seeds `memories/repo/*.md` with generic kit content. Run each step interactively by prompting the user for details to rewrite them:
    ...
    - **Domain Packs**: Read `memories/domain/glossary.md` triggers. Prompt if subdomains exist. If yes, copy `memories/domain/example/` to `memories/domain/<name>/` as a scaffold, then pre-fill trigger keywords and update content from the adopter's answers. Update `MASTER_INDEX.md` in either case.
