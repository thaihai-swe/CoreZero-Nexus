...
Brownfield mode is not a substitute for `/brownfield-init`. The archaeology pass is
required first. `starter-init` only continues after the brownfield memory pack exists.
...
4. **Brownfield pack verification**: Confirm `memories/repo/brownfield/brownfield-map.md` and `memories/repo/brownfield/dependency-graph.md` already exist. If either is missing, stop and route back to `/brownfield-init`.
...
- The repository is brownfield but the archaeology pack is missing — do not continue until `/brownfield-init` runs.
