...
2. **AC Traceability Check** — for each AC in `spec.md`:
   ...
   If any AC has no corresponding `Done` task, or the gate failed, block and route back to `/spec-implement` with a list of unresolved ACs. (Consult `references/definition-of-done.md` for final validation.)
