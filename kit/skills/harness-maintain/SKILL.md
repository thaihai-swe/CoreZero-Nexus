...
- Reads: Entire codebase, `core-zero/memories/repo/`, `manifest.json`, `harness-telemetry.md`.
- Writes: `core-zero/generated/harness-assessment.md`, `core-zero/memories/repo/learned-heuristics.md`, `core-zero/project/code-map.md`, `core-zero/generated/dashboard.html`.
...
1. Read all files in `core-zero/memories/repo/`. Count line lengths. Flag any file > 600 lines as a warning, > 800 lines as a critical compaction risk.
2. Check for orphaned feature artifacts (e.g., a directory in `artifacts/features/` with no `status.md`).
3. Check for missing required sections in `core-zero/memories/repo/core-policies.md`.
...
2. Scaffold any missing standard directories (e.g., `core-zero/memories/domain/`, `core-zero/generated/`).
...
1. Read `core-zero/memories/repo/harness-telemetry.md` `## Entries`.
...
4. Append the draft to `core-zero/memories/repo/learned-heuristics.md` clearly marked as `[DRAFT]`.
...
### 4. Eval Mode (`eval`)
1. Verify all `CC-*` identifiers defined in `core-zero/memories/repo/core-policies.md` are present and sequential.
